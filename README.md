...
3) Walking animation - This very basic sprite animation was built with nothing but React. I used a larger image containing every frame of the character's walking animation and used a React state and CSS's "overflow: hidden" property to only show one frame/snippet of that image at a time. Note - I am only switching the sprite frame on each character render here. There are other ways (e.g., react-transition-group, CSS transitions) to add smoother transitions if needed.
